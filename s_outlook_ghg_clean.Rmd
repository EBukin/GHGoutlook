...
The purpose of this document is to explain the process of reproducing the GHG
 ...
 *   GB, GH and GA domains are projected as a constant share of total emissions
     ...
     period.  
 *   GY domain emissions data are approximated based on the area and yields of 
 ...
 *   GV domain data is kept at the constant level as it is assumed in the 
     FAOSTAT. Alternatively, we test a situation, when emissions from the organic 
     soils are changing with the same rate as the area utilized under the 
     palm oil production.  
     
 Below, we elaborate more explicitly on the methodology of the GHG estimation 
...
## Outlook data
 ...
 First we load all outlook data. If there is no data save in the Rdata file we reload all data from the CSV file.
...
Next step is loading all FAOSTAT data. Since FAOSTAT data combines data from 
  multiple domains, we load it all in on .Rdata file. In case if there is no such file,
 ...
 *  GP - Manure left of pastures
  ...
  The overall process consist of several important steps. All steps are 
      organised in the body of a function `outlook_emissions`. This function 
      utilities faostat data, outlook data and previously loaded mapping tables
      for reproducing emissions for the pre-defined domain. The steps of 
      reproduction are the following:
  
  1.  Mapping FAOSTAT Areas to the outlook regions re-estimating activity data 
      and emissions respectively. Mapping the FAOSTAT activity data to the 
      outlook activity data aggregating FAOSTAT items to the outlook items 
      and re-estimating emissions and activity data according to aggregating. 
      This is done with the `map_fs_data` function, which uses items and elements
      mapping tables and faostat filtered to one domain data. Thing the function
      ...
      items and elements may be aggregated by subtracting one from another what 
  ...
  3.  Adjusting outlook activity data to the baseline level derived from the 
      ...
      function, where mapped faostat data is used for subset the outlook data 
      to the items and elements relevant for one domain with the function 
      `subset_outlook`. After sub-setting, we apply function `adjust_outlook_activity`
      in order to adjust activity data from the outlook to the levels of the
      FAOSTAT in the historical period.    
   
  4.  At the next step we `reestimate_emissions` data based on the activity 
  ...
  5.  In some cases, for some items and elements outlook does not have any 
      ...
      these items and elements in the known and estimated emissions. 
      Constant share is assumed based on the 5 years average share calculated 
      on the last available. This step is made with the function `estimate_missing_emissions`.  
      
  6.  At the next step we convert all GHG to the GHG expressed in the CO2
      equivalent with the function `convert_ghg`.
      
  7.  After the numbers are re-estimated in the steps 1-4, we aggregate regions
...
We preform all above explained calculations for one domain at the time. That allows 
...
For the GV - Cultivating Organic Soils domain we repeat the last know values.
...
Reproducing emissions for the domains Burning crop residues, Burning Savanna 
  and crop residues. To reproduce emissions for these domains such we use 
  the constant share of the emissions from this domains in the estimable 
...
Combining Land use total emissions
...
Structure of the exported data is similar:

```{r }
str(activityBasedData)
```

Important to notice that in any of these data frames, variable `d.source` 
  ...
  *  Outlook - stands for the outlook based data with the adjusted activity data, 
     whenever it was not properly matching with the FAOSTAT
